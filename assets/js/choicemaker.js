/****************************************************
Originally forked from https://github.com/zhm/csvtool
****************************************************/

var csName, csDescription;
$(function() {
  ChoiceMaker = {};
  ClassMaker = {};
  ...
  ClassMaker.convert = function() {
    ...
    var classes = [];
    var classificationSet = currentNode = { name: csName ? csName : '', description: csDescription ? csDescription : '', items: [] };
    var currentIndent = 1;
    ...
    var recurse = function(items) {
      ...
      $.each(result, function(i, item) {
        if ($.isEmptyObject(item.child_classifications)) {
          delete result[i].child_classifications;
        }
      });
      return result;
    };
    ...
    formattedObject = {'classification_set': withoutCircularReferences};

    $('.right-text').val(JSON.stringify(formattedObject, null, '  '));
  };
  ...
  ClassMaker.convertToCSV = function() {
    ...
    try {
      ...
      if (json.classification_set) {
        csName = json.classification_set.name;
        csDescription = json.classification_set.description;
